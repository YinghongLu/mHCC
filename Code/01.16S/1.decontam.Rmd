...
p_LibrarySize_B1 <- ggplot(data=df,aes(x=Group3, y=LibrarySize)) +
  ...
  #guides(fill= "Group") +
  #guides(fill="none")+
  #geom_signif(annotations="P=", y_position=, xmin=1, xmax=2, tip_length=0.045, textsize=5) +
...
p_LibrarySize_B2 <- ggplot(data=df, aes(x=Index, y=LibrarySize, color=Group3)) + 
  ...
  geom_point(data = subset(df, Group3 == "Control"), aes(x=Index, y=LibrarySize), color = "#bf5b17", size = 0.5) + # 突出A组
  scale_color_manual(values=c("#d95f02","#1b9e77"))+
  scale_y_continuous(labels = label_scientific()) + 
  # geom_label_repel(aes(x=Index, y=LibrarySize, label=df$Group3))+
  theme(panel.background=element_rect(fill="white"),
        panel.border = element_rect(fill=NA,size=0.1),
        axis.title.x=element_blank(),
        axis.title.y=element_text(size=14),
...
table(contamdf.prev05$contaminant) #1744
