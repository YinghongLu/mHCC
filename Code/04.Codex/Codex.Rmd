...
DataFilter2 <- DataFilter2[apply(DataFilter2[, 1:16], 1, function(cell) {
  all(cell <= marker_thresholds)  # 判断每个细胞是否都在阈值以下
}), ]
...
p = DimPlot(codex_F, reduction = "umap", group.by = "sample_name", order = rev(unique(codex_F@meta.data$sample_name))) ;p #+ NoLegend()
...
pdf("../03.Output/FilterID2/codex_F_FilterID_umap_split.pdf", width = 8, height = 4)
p
dev.off()
...
p = DimPlot(codex_F_com, reduction = "umap", group.by = "sample_name", order = rev(unique(codex_F_com@meta.data$sample_name))) ;p #+ NoLegend()
...
levels(codex_F_com_phenograph.seurat) = as.character(1:52) # to order the top annotation order of the following heatmap
...
pdf("../03.Output/FilterID_/codex_F_com_FilterID_F_Celltype_Umap_Split.pdf", width = 13, height = 8)
p
dev.off()
...
dt <- codex_F_com@assays$CODEX$scale.data #这里以Top5上调为例
dtt <- expm1(dt)
...
col_fun = circlize::colorRamp2(c(-0.8, 0, 2), c('#5E3C99', "white", '#E66101')) 

#图表个性化调整:
p2 <- p1 +
  scale_size_continuous(range = c(2,6)) + #气泡大小范围调整
  # scale_color_gradient2(low='#5E3C99', high='#E66101', mid = 'white') + 
